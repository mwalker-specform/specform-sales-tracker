...
def list_glassworks_quotes(
    ...
    with get_db() as con:
        sql = "SELECT * FROM glassworks_quotes WHERE 1=1"
        params = []
        if status and status != "All":
            sql += " AND status = ?"
            params.append(status)
...
def get_glassworks_quote(quote_id: int):
    with get_db() as con:
        row = con.execute("SELECT * FROM glassworks_quotes WHERE id=?", (quote_id,)).fetchone()
    if not row:
...
def create_glassworks_quote(q: QuoteIn):
    with get_db() as con:
        ...
        INSERT INTO glassworks_quotes
            (status,date_received,date_quoted,sent_to,subject,job_name,
             customer,location,product,price,quantities,amount,
             close_date,est_freight,lead_time,notes,add_to_salesforce,completed)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        """, (q.status, normalize_date(q.date_received), normalize_date(q.date_quoted),
              q.sent_to, q.subject, q.job_name, q.customer, q.location, q.product,
              q.price, q.quantities, q.amount, normalize_date(q.close_date),
              q.est_freight, q.lead_time, q.notes,
              q.add_to_salesforce or 0, q.completed or 0))
    return {"id": cur.lastrowid}
...
def update_glassworks_quote(quote_id: int, q: QuoteIn):
    with get_db() as con:
        ...
        """, (q.status, normalize_date(q.date_received), normalize_date(q.date_quoted),
              ...
              q.price, q.quantities, q.amount, normalize_date(q.close_date),
              q.est_freight, q.lead_time, q.notes,
              q.add_to_salesforce or 0, q.completed or 0, quote_id))
    return {"ok": True}

@app.delete("/api/glassworks-quotes/{quote_id}")
def delete_glassworks_quote(quote_id: int):
    with get_db() as con:
        con.execute("DELETE FROM glassworks_quotes WHERE id=?", (quote_id,))
    return {"ok": True}
...
def export_glassworks_quotes():
    ...
    with get_db() as con:
        rows = con.execute("""
            SELECT id, status, date_received, date_quoted, sent_to, subject,
                   ...
                   amount, close_date, est_freight, lead_time, notes,
                   add_to_salesforce, completed
            FROM glassworks_quotes ORDER BY date_received DESC
        """).fetchall()
    ...
    header_font  = Font(name='Calibri', bold=True, color='FFFFFF', size=11)
    header_fill  = PatternFill('solid', fgColor='7a5c9e')   # purple for Glassworks
    header_align = Alignment(horizontal='center', vertical='center', wrap_text=True)
    border_side  = Side(style='thin', color='BFBFBF')
    cell_border  = Border(left=border_side, right=border_side,
                          top=border_side, bottom=border_side)
    zebra_fill   = PatternFill('solid', fgColor='F3E8FF')
    STATUS_FILLS = {
        'Won':    PatternFill('solid', fgColor='D9EAD3'),
        'Lost':   PatternFill('solid', fgColor='F4CCCC'),
        'Verbal': PatternFill('solid', fgColor='FFF2CC'),
...
def glassworks_sync():
    """Pull emails from 'Glassworks Quotes' Outlook folder via Graph API."""
    if not GRAPH_CLIENT_SECRET:
    ...
    # Find the "Glassworks Quotes" folder (check top-level and Inbox children)
    folder_id = None
    for list_url in [
    ...
    # Paginate through messages (up to 200)
    msgs = []
    next_url = (f"https://graph.microsoft.com/v1.0/users/{GRAPH_USER}"
                f"/mailFolders/{folder_id}/messages"
                f"?$select=from,toRecipients,subject,receivedDateTime&$top=50")
    ...
    with get_db() as con:
        for msg in msgs:
            ...
            if not subject.lower().startswith("glasswork quote"):
                skipped += 1
                continue
    ...
    with get_db() as con:
        ...
        by_month = con.execute("""
            SELECT substr(date_received,1,7) as month,
                ...
                COUNT(CASE WHEN status='Won'    THEN 1 END) as won_count,
                COUNT(CASE WHEN status='Verbal' THEN 1 END) as verbal_count
            FROM glassworks_quotes WHERE date_received IS NOT NULL AND date_received != ''
